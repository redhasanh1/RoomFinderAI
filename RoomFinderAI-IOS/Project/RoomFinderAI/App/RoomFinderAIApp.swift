...
struct AINegotiatorView: View {
    ...
    private func sendInitialMessage() async {
        ...
        let aiMessage = NegotiationMessage(
            id: UUID(),
            content: "Hello! I'm here to help you negotiate the price for \(session.roomTitle). The asking price is $\(String(format: "%.0f", session.room.price)). What would you like to offer?",
            isFromUser: false,
            timestamp: Date(),
            offer: nil
        )
