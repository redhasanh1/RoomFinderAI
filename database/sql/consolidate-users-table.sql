...
-- Make user_id required in favorites
ALTER TABLE favorites 
ALTER COLUMN user_id SET NOT NULL;

-- Drop the old user_email column from favorites
ALTER TABLE favorites 
...
SET user_id = u.id,
    landlord_id = ul.id
FROM users u, users ul
WHERE bookings.user_email = u.email
AND bookings.landlord_email = ul.email;
...
-- Add foreign key constraints now that data is properly linked
ALTER TABLE favorites 
...
-- ============================================
-- STEP 6: Remove email columns where no longer needed
-- ============================================

-- Clean up email-based references (keep some for backwards compatibility)
ALTER TABLE bookings DROP COLUMN IF EXISTS user_email;
ALTER TABLE bookings DROP COLUMN IF EXISTS landlord_email;
ALTER TABLE reviews DROP COLUMN IF EXISTS user_email;
ALTER TABLE notifications DROP COLUMN IF EXISTS user_email;
ALTER TABLE search_history DROP COLUMN IF EXISTS user_email;
ALTER TABLE user_preferences DROP COLUMN IF EXISTS user_email;
ALTER TABLE listing_views DROP COLUMN IF EXISTS user_email;

-- Keep user_email in listings for now (backwards compatibility)
-- ALTER TABLE listings DROP COLUMN IF EXISTS user_email;

-- ============================================
-- STEP 7: Create updated indexes
-- ============================================
...
BEGIN
    ...
    RAISE NOTICE 'Migration complete! profiles table can be dropped when ready.';
END $$;

-- ============================================
-- STEP 10: Prepare to drop profiles table (commented out for safety)
-- ============================================
