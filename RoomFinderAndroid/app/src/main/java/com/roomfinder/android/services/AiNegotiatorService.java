...
public class AiNegotiatorService {
    ...
    private static final String WEB_CHAT_URL = "https://www.roomfinderai.com/api/chat";

    private boolean hasOpenAiKey() {
        String key = ApiKeys.OPENAI_API_KEY;
        return key != null
                && !key.isEmpty()
                && !"your_openai_api_key_here".equals(key)
                && !"API_KEY_NOT_CONFIGURED".equals(key);
    }

    /**
     * Same endpoint the website's negotiator uses: POST {message} and read back
     * {response}. No credentials travel with the app.
     */
    private void sendViaWebApi(String userMessage, AiResponseCallback callback) {
        String sanitized = userMessage == null ? "" : userMessage.trim();
        ...
        if (retryCount == 0) {
            conversationHistory.add(new ChatMessage("user", sanitized));
        }

        try {
            JSONObject body = new JSONObject();
            body.put("message", sanitized);
            ...
            client.newCall(request).enqueue(new Callback() {
                ...
                public void onFailure(Call call, IOException e) {
                    ...
                    callback.onError("Couldn't reach the negotiator. Check your connection.");
                }
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    try (ResponseBody responseBody = response.body()) {
                        if (!response.isSuccessful() || responseBody == null) {
                            Log.e(TAG, "Chat API returned HTTP " + response.code());
                            callback.onError("The negotiator is unavailable right now.");
                            return;
                        }
                        String raw = responseBody.string();
                        JSONObject json = new JSONObject(raw);
                        String reply = json.optString("response", "").trim();
                        if (reply.isEmpty()) {
                            reply = json.optString("message", "").trim();
                        }
                        if (reply.isEmpty()) {
                            callback.onError("The negotiator had nothing to say. Try rephrasing.");
                            return;
                        }
                        conversationHistory.add(new ChatMessage("assistant", reply));
                        callback.onSuccess(new AiResponse(reply, true));
                    } catch (Exception e) {
                        Log.e(TAG, "Failed to parse chat response", e);
                        callback.onError("Couldn't read the negotiator's reply.");
                    }
                }
            });
        } catch (Exception e) {
            Log.e(TAG, "Failed to build chat request", e);
            callback.onError("Couldn't send that message.");
        }
    }
    ...
    private void processMessageWithRetry(String userMessage, AiResponseCallback callback) {
        ...
        // No key in the APK? Go through the website's own /api/chat, which is
        // what the iOS app does and what the site does. Calling OpenAI straight
        // from the client would mean shipping a secret key inside the APK, and
        // without one the app's headline feature just answered "not configured".
        if (!hasOpenAiKey()) {
            Log.d(TAG, "No OpenAI key configured - using the roomfinderai.com chat API");
            sendViaWebApi(userMessage, callback);
            return;
        }
