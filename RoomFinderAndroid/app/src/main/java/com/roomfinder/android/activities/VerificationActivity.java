...
public class VerificationActivity extends AppCompatActivity {
    ...
    private void showDemoModeIfNeeded() {
        ...
        //
        // This block can print a six digit verification code on screen and, for
        // three hardcoded addresses, offer a "Skip Verification (Demo Mode)"
        // button that marks an account verified without the server ever
        // agreeing. It is dormant in production today only because the live
        // API does not put the code in its response - the register call answers
        // {"message":"Verification code sent to your email"} and nothing more.
        //
        // That is a thin thing to rely on. If any backend ever includes a code
        // in a message, this screen would display a stranger's verification
        // code to whoever typed their address. The debug line underneath prints
        // the code and the email too. None of it belongs in the app people
        // download, and Play reviewers are right to be suspicious of visible
        // debug affordances, so it is compiled out of release entirely and left
        // working for local development.
        if (!com.roomfinder.android.BuildConfig.DEBUG) {
    ...
    private void showDebugInfo(String info) {
        // The text includes the verification code and the email address, so it
        // stays out of release builds along with the rest of the demo path.
        if (!com.roomfinder.android.BuildConfig.DEBUG) {
            return;
        }
