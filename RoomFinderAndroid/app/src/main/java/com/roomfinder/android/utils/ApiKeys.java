...
public class ApiKeys {
    ...
    /**
     * True when a usable Supabase anon key is configured (local.properties or env).
     * When false, callers must use the public roomfinderai.com API instead of
     * hitting Supabase REST directly -- an unauthenticated PostgREST call returns
     * 401 "No API key found in request" and looks to the user like an empty app.
     */
    public static boolean hasSupabaseKey() {
        return SUPABASE_ANON_KEY != null
                && !SUPABASE_ANON_KEY.trim().isEmpty()
                && !"API_KEY_NOT_CONFIGURED".equals(SUPABASE_ANON_KEY);
    }
