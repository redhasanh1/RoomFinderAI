...
public class AuthService {
    ...
    public void sendPasswordResetCode(String email, ResetCodeCallback callback) {
        sendPasswordResetCode(email, null, callback);
    }

    /**
     * @param turnstileToken Cloudflare token. The endpoint answers
     *                       "Bot verification required" without one, exactly as
     *                       it does for the website.
     */
    public void sendPasswordResetCode(String email, String turnstileToken, ResetCodeCallback callback) {
        ...
        new Thread(() -> {
            try {
                ...
                if (turnstileToken != null && !turnstileToken.isEmpty()) {
                    requestData.put("turnstileToken", turnstileToken);
                }
