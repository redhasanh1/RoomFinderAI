...
public class AiChatFragment extends Fragment {
    ...
    /** Question to open with, when the negotiator was launched from a room. */
    public static final String ARG_SEED_MESSAGE = "seed_message";
    ...
    /**
     * The primer covers the transcript while it is empty: what the negotiator
     * is, and three prompts you can send with one tap. It hides itself as soon
     * as there is a conversation to read.
     */
    /** Pre-fills the composer when the negotiator was opened about a room. */
    private void applySeedMessage() {
        if (binding == null || getArguments() == null) {
            return;
        }
        String seed = getArguments().getString(ARG_SEED_MESSAGE);
        if (seed != null && !seed.trim().isEmpty()) {
            binding.messageInput.setText(seed);
            binding.messageInput.setSelection(seed.length());
        }
    }

    private void setupPrimer() {
        if (binding == null) {
            return;
        }
        binding.suggestionOne.setOnClickListener(v -> sendSuggestion(binding.suggestionOne));
        binding.suggestionTwo.setOnClickListener(v -> sendSuggestion(binding.suggestionTwo));
        binding.suggestionThree.setOnClickListener(v -> sendSuggestion(binding.suggestionThree));
        updatePrimerVisibility();
    }

    private void sendSuggestion(android.widget.TextView suggestion) {
        sendMessage(suggestion.getText().toString());
    }

    private void updatePrimerVisibility() {
        if (binding == null || binding.negotiatorPrimer == null) {
            return;
        }
        boolean empty = chatAdapter == null || chatAdapter.getItemCount() == 0;
        binding.negotiatorPrimer.setVisibility(empty ? View.VISIBLE : View.GONE);
    }

    private void sendMessage(String messageText) {
        // Asking a question needs no account: the website answers anonymously
        // through the same /api/chat endpoint, so refusing here only made the
        // app look broken next to its own site. Sign-in is still required for
        // the things that need an identity - saving a conversation, or opening
        // a negotiation with a named landlord.
        ...
        try {
            ...
            if (savedHistory != null && !savedHistory.isEmpty()) {
                ...
                if (!this.messages.isEmpty()) {
                    ...
                    // Restoring a conversation has to put the primer away too.
                    // It did not, so reopening the negotiator drew the welcome
                    // panel - icon, blurb and all three suggestion chips - on
                    // top of the restored messages, and the two were legible
                    // through each other. Sending hid it, so this only ever
                    // showed on the way back in, which is why it survived.
                    updatePrimerVisibility();
                    scrollToBottom();
