...
public class SupabaseService {
    ...
    /** For calls whose answer is just "it worked" or "it didn't". */
    ...
    public void refreshListings(ListingsCallback callback) {
        ...
        // Pull-to-refresh has to reach the network, not the burst cache that
        // exists to collapse the progressive loader's duplicate calls.
        WebApiListingsSource.getInstance().invalidateCache();
        fetchListingsFromNetwork(callback);
    }
    ...
    /**
     * Takes a listing down for everyone.
     *
     * Uses the same endpoint the website's profile page uses. The server checks
     * that the listing belongs to this email before deleting, so a wrong or
     * missing address comes back as 403 rather than removing someone else's
     * room.
     */
    public void deleteListing(String listingId, String userEmail, SimpleCallback callback) {
        if (listingId == null || userEmail == null || userEmail.isEmpty()) {
            callback.onError("Missing listing or account details");
            return;
        }
        new Thread(() -> {
            ...
            try {
                okhttp3.Request request = new okhttp3.Request.Builder()
                        .url("https://www.roomfinderai.com/api/listings/" + listingId)
                        .header("User-Agent", com.roomfinder.android.network.AppUserAgent.VALUE)
                ...
                try (okhttp3.Response response = client.newCall(request).execute()) {
                    ...
                    if (response.isSuccessful()) {
                        // The feed caches listings for a short burst; without
                        // this the deleted room reappears on the next screen.
                        com.roomfinder.android.network.WebApiListingsSource.getInstance().invalidateCache();
                        new android.os.Handler(android.os.Looper.getMainLooper())
                                .post(callback::onSuccess);
                    } else {
                        String message = "Couldn't delete that listing";
                        try {
                            org.json.JSONObject error = new org.json.JSONObject(payload);
                        ...
                        new android.os.Handler(android.os.Looper.getMainLooper())
                                .post(() -> callback.onError(finalMessage));
                    }
                }
            } catch (Exception e) {
                new android.os.Handler(android.os.Looper.getMainLooper())
                        .post(() -> callback.onError("Couldn't reach the server. Check your connection."));
            }
        }).start();
    }
}