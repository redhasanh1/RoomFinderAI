...
import com.roomfinder.android.network.WebApiListingsSource;
...
public class SupabaseClient {
     ...
     */
    private List<Listing> getAllListingsFromSupabase() {
        try {
     ...
     */
    private List<Listing> getListingsFromSupabase(int offset, int limit) {
        try {
     ...
     */
    private List<Listing> getMoreListingsFromSupabase(int offset, int limit) {
        try {
            String url = baseUrl + "listings?select=*&order=created_at.desc&limit=" + limit + "&offset=" + offset;
     ...
     */
    private List<Listing> searchListingsFromSupabase(String query) {
        try {
     ...
     */
    private List<Listing> filterListingsFromSupabase(Integer minPrice, Integer maxPrice, Integer bedrooms, String propertyType, String location) {
        try {
     ...
     */
    private List<Listing> filterListingsEnhancedFromSupabase(Integer minPrice, Integer maxPrice, Integer bedrooms, 
     ...
     */
    private Listing getListingByIdFromSupabase(String listingId) {
        try {
    ...
    // Public entry points.
    //
    // Supabase REST is used only when an anon key is actually configured;
    // otherwise (and whenever a direct query comes back empty) the request is
    // served by the public roomfinderai.com API, which needs no credentials and
    // returns the same JSON shape the app's Listing model expects.
    // ------------------------------------------------------------------

    private WebApiListingsSource webApi() {
        return WebApiListingsSource.getInstance();
    }

    public List<Listing> getAllListings() {
        if (ApiKeys.hasSupabaseKey()) {
            List<Listing> listings = getAllListingsFromSupabase();
            if (!listings.isEmpty()) {
                return listings;
            }
            Log.w(TAG, "Supabase returned no listings - falling back to web API");
        }
        return webApi().getAllListings();
    }

    public List<Listing> getListings(int offset, int limit) {
        if (ApiKeys.hasSupabaseKey()) {
            List<Listing> listings = getListingsFromSupabase(offset, limit);
            if (!listings.isEmpty()) {
                return listings;
            }
            Log.w(TAG, "Supabase page was empty - falling back to web API");
        }
        return webApi().getListings(offset, limit);
    }

    public List<Listing> getMoreListings(int offset, int limit) {
        if (ApiKeys.hasSupabaseKey()) {
            List<Listing> listings = getMoreListingsFromSupabase(offset, limit);
            if (!listings.isEmpty()) {
                return listings;
            }
        }
    ...
    public List<Listing> searchListings(String query) {
        if (ApiKeys.hasSupabaseKey()) {
            List<Listing> listings = searchListingsFromSupabase(query);
            if (!listings.isEmpty()) {
                return listings;
            }
        }
        return webApi().searchListings(query);
    }

    public List<Listing> filterListings(Integer minPrice, Integer maxPrice, Integer bedrooms,
                                        String propertyType, String location) {
        if (ApiKeys.hasSupabaseKey()) {
            List<Listing> listings = filterListingsFromSupabase(minPrice, maxPrice, bedrooms, propertyType, location);
            if (!listings.isEmpty()) {
                return listings;
            }
        }
        return filterLocally(webApi().getAllListings(), minPrice, maxPrice, bedrooms, null, propertyType, location);
    }

    public List<Listing> filterListingsEnhanced(Integer minPrice, Integer maxPrice, Integer bedrooms,
                                                Integer bathrooms, String propertyType, String location) {
        if (ApiKeys.hasSupabaseKey()) {
            List<Listing> listings = filterListingsEnhancedFromSupabase(minPrice, maxPrice, bedrooms,
                    bathrooms, propertyType, location);
            if (!listings.isEmpty()) {
                return listings;
            }
        }
        return filterLocally(webApi().getAllListings(), minPrice, maxPrice, bedrooms, bathrooms, propertyType, location);
    }

    public Listing getListingById(String listingId) {
        if (ApiKeys.hasSupabaseKey()) {
            Listing listing = getListingByIdFromSupabase(listingId);
            if (listing != null) {
                return listing;
            }
        }
        return webApi().getListingById(listingId);
    }

    /** Applies the same filter semantics as the PostgREST query, client side. */
    private List<Listing> filterLocally(List<Listing> listings, Integer minPrice, Integer maxPrice,
                                        Integer bedrooms, Integer bathrooms, String propertyType, String location) {
        List<Listing> matches = new ArrayList<>();
        String type = propertyType == null ? null : propertyType.trim().toLowerCase();
        String place = location == null ? null : location.trim().toLowerCase();

        for (Listing listing : listings) {
            if (listing == null) {
                continue;
            }
            double price = listing.getSafePrice();
            ...
            if (bedrooms != null && listing.getBedrooms() != bedrooms) {
                continue;
            }
            if (bathrooms != null && listing.getBathrooms() != bathrooms) {
                continue;
            }
            if (type != null && !type.isEmpty() && !"all".equals(type)) {
                String listingType = listing.getHouseType();
                if (listingType == null || !listingType.toLowerCase().contains(type)) {
                    continue;
                }
            }
            if (place != null && !place.isEmpty()) {
                String city = listing.getCity() == null ? "" : listing.getCity().toLowerCase();
                String street = listing.getStreet() == null ? "" : listing.getStreet().toLowerCase();
                if (!city.contains(place) && !street.contains(place)) {
                    continue;
                }
            }
            matches.add(listing);
        }
        Log.d(TAG, "Local filter matched " + matches.size() + " of " + listings.size() + " listings");
        return matches;
    }
}
