...
public class ListingsAdapter extends RecyclerView.Adapter<ListingsAdapter.ListingViewHolder> {
    ...
    class ListingViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(Listing listing, int position) {
            ...
            binding.priceText.setText(String.format(Locale.US, "$%,.0f/mo", listing.getSafePrice()));
            
            // Bed/bath only: the property type is shown once, as a badge on the
            // photo. Repeating it here used to push the line onto three rows and
            // hyphenate words like "Tow nhouse".
            String bedLabel = listing.getBedrooms() == 0
                    ? "Studio"
                    : String.format(Locale.US, "%d bed", listing.getBedrooms());
            binding.bedBathText.setText(String.format(Locale.US, "%s • %d bath",
                    bedLabel, listing.getBathrooms()));
