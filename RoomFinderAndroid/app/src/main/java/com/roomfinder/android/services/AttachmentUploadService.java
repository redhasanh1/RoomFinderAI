...
public class AttachmentUploadService {
    ...
    public void uploadListingPhoto(Uri fileUri, String fileName, UploadCallback callback) {
        // Supabase storage rejects an unauthenticated PUT with 400, so without a
        // configured anon key the photo never attached to the listing even
        // though the AI had just read it. The website's own upload endpoint
        // takes multipart and needs no credentials, exactly as iOS does it.
        if (!com.roomfinder.android.utils.ApiKeys.hasSupabaseKey()) {
            uploadViaWebApi(fileUri, fileName, callback);
            return;
        }
        uploadToBucket(fileUri, "listing-media", "Photos/" + fileName, callback);
    }

    /** POST multipart "photos" to /api/listings/photos, read back {urls:[...]}. */
    private void uploadViaWebApi(Uri fileUri, String fileName, UploadCallback callback) {
        new Thread(() -> {
            try {
                byte[] bytes = readBytes(fileUri);
                if (bytes == null) {
                    postError(callback, "That photo could not be read");
                    return;
                }
                notifyProgress(callback, 30);

                okhttp3.RequestBody filePart = okhttp3.RequestBody.create(
                        bytes, okhttp3.MediaType.parse("image/jpeg"));
                okhttp3.MultipartBody body = new okhttp3.MultipartBody.Builder()
                        .setType(okhttp3.MultipartBody.FORM)
                        .addFormDataPart("photos", fileName, filePart)
                        .build();

                okhttp3.Request request = new okhttp3.Request.Builder()
                        .url("https://www.roomfinderai.com/api/listings/photos")
                        .post(body)
                        .build();

                okhttp3.OkHttpClient uploader = new okhttp3.OkHttpClient.Builder()
                        .connectTimeout(20, java.util.concurrent.TimeUnit.SECONDS)
                        .writeTimeout(120, java.util.concurrent.TimeUnit.SECONDS)
                        .readTimeout(120, java.util.concurrent.TimeUnit.SECONDS)
                        .build();

                try (okhttp3.Response response = uploader.newCall(request).execute()) {
                    String raw = response.body() == null ? "" : response.body().string();
                    if (!response.isSuccessful()) {
                        Log.e(TAG, "Photo upload failed: HTTP " + response.code() + " " + raw);
                        postError(callback, "Your photo could not be uploaded");
                        return;
                    }
                    org.json.JSONObject json = new org.json.JSONObject(raw);
                    org.json.JSONArray urls = json.optJSONArray("urls");
                    ...
                    new android.os.Handler(android.os.Looper.getMainLooper())
                            .post(() -> callback.onSuccess(url, fileName, "image/jpeg"));
                }
            } catch (Exception e) {
    ...
    private byte[] readBytes(Uri fileUri) {
        try (java.io.InputStream in = context.getContentResolver().openInputStream(fileUri)) {
            ...
            Log.e(TAG, "Could not read photo", e);
            return null;
        }
    }

    private void notifyProgress(UploadCallback callback, int percent) {
        new android.os.Handler(android.os.Looper.getMainLooper())
