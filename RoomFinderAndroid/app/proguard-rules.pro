...
# ---------------------------------------------------------------------------
# Gson reads generic types at runtime through anonymous TypeToken subclasses:
#
#     new TypeToken<List<Listing>>(){}.getType()
#
# R8 is free to merge or strip those anonymous classes and their generic
# signatures. When it does, getType() degrades to a raw type, Gson hands back
# LinkedTreeMap instead of Listing, and the parse fails - quietly, because the
# failure is caught and turned into an empty list. The release build then shows
# "No rooms found" on a working API while the debug build shows every room.
# ---------------------------------------------------------------------------
-keepattributes Signature, InnerClasses, EnclosingMethod
...
# Keep logging in release builds while the app is still being verified. Without
# this every diagnostic disappears and an empty screen has no explanation.
# Remove the -dontnote once the release build is trusted.
