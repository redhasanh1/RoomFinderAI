...
**Not started yet:** §5a–5d (listings/photos/map) partially addressed — Add Listing nav link fixed, photo upload uses session JWT, message popup layout fixed, listing inbox shows counterparty names from profiles. §7 (AI notifications) partially — sublease interest now writes to ai_chats. §8 (student housing) — university selector now cascades to budget, agreement, and housing results. §9 (sublease reverse interest) — Interest Received tab + `/api/sublease/interests`. §10 (AI negotiator) — duplicate `chatMessages` id fixed. §11/§14 (Legal Help / AI Assessment), §12 (Stripe) — billing section on profile + expanded webhook + customer portal. §13 (Support Center), §15 (nav bar) — profile text link hidden in shared nav.
