...
Since the foreign key constraint `users_id_fkey` is blocking programmatic user creation, please run this manually in your Supabase SQL Editor:

## Step 1: Go to Supabase SQL Editor
1. Open your Supabase dashboard: https://fkktwhjybuflxqzopaex.supabase.co
2. Navigate to the SQL Editor tab
3. Create a new query

## Step 2: Run This SQL (Copy and paste exactly)
...
## Step 3: After successful migration, run this linking query

```sql
-- Update listings to reference users by ID instead of email
UPDATE listings 
SET user_id = u.id
FROM users u
...
-- Update messages to reference users by ID
UPDATE messages
SET user_id = u.id
FROM users u  
...
SELECT 
    ...
    (SELECT COUNT(*) FROM listings WHERE user_id IS NOT NULL) as connected_listings,
    (SELECT COUNT(*) FROM listings) as total_listings;
```

## Expected Results
- **64 users** in users table (migrated from profiles)
- **56 listings** connected to users by ID
- All website functions saving properly to database

Once this is complete, all your data will be properly connected and optimized!